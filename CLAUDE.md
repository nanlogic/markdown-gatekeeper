...
2. Run `mdg status .`. If the global command is unavailable, use the managed launcher at `<Claude home>/bin/mdg` (or `mdg.cmd` on Windows). Only if that launcher is unavailable, first verify that `./bin/mdg.mjs` exists and then use `node ./bin/mdg.mjs`; do not run a predictable failing command to probe availability.
...
6. If status reports registry revision 0 or pending legacy entrypoints, automatically run `mdg init . --host-session claude --yes` (or the repository-local CLI) and complete any current-session review handoff without asking the user to repeat initialization.
